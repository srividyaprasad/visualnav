...
output_video_path = f"{OUTPUT_IMG_PATH}/navigate_nomad_{datetime.now().strftime('%Y_%m_%d_%H_%M_%S')}.mp4"  # Output video file path
output_video_path_vint = f"{OUTPUT_IMG_PATH}/navigate_vint_{datetime.now().strftime('%Y_%m_%d_%H_%M_%S')}.mp4"  # Output video file path
...
video_writer_action = cv2.VideoWriter(output_video_path, fourcc, fps, (frame_width, frame_height))
video_writer_waypoints = cv2.VideoWriter(output_video_path_vint, fourcc, fps, (frame_width, frame_height))
...
def visualize_network_action(img, actions, frame_id=0):
    ...
    for i, curve_points in enumerate(actions):
        ...
        video_writer_action.write(background)
        first_candidate = False

def visualize_waypoints(img, waypoints, idx, frame_id=0):
    ...
    for i, waypoint in enumerate(waypoints):
        projected_points, _ = cv2.projectPoints(waypoint, rvec, tvec, K, distCoeffs)
        projected_points = projected_points.reshape(-1, 2) # Reshape to (num_points, 2)
        # --- Draw the Curve on the Background Image ---
        # Draw small circles at each projected point
        for point in projected_points:
        ...
        # --- Save the Final Image to Disk ---
        # cv2.imwrite(f"{OUTPUT_IMG_PATH}/out_{frame_id}.png", background)
        video_writer_waypoints.write(background)
